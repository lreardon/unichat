...
from unicrawl.normalization.is_same_domain import is_same_domain
...
async def run_crawl(config: CrawlConfig) -> CrawlResult:
    ...
    output_dir = Path("universities") / university_name
    ...
    logger.info(
        "crawl.context root_url={} output_dir={} force={} timeout_s={} mode=autoscale objective=throughput initial_pool_size={} internal_min_pool_size={} monitor_interval_s={} scale_up_step={} scale_down_step={}",
        root_url,
        output_dir,
        config.force,
    ...
    def collect_outgoing_links(html: str, page_url: str) -> list[str]:
        ...
        for link in extract_links(html, page_url):
            ...
            if not candidate:
                continue
            if not is_same_domain(root_url, candidate):
                continue
        ...
        while True:
            ...
            try:
                ...
                if config.max_depth is not None and depth > config.max_depth:
                    ...
                    continue
                if not is_same_domain(root_url, normalized_url):
                    log_timing(
                        "validation",
                        total_ms=(time.perf_counter() - validation_started_at) * 1000.0,
                        worker_id=worker_id,
                ...
                if resolved_normalized_url and resolved_normalized_url != normalized_url:
                    ...
                    enqueue_count = 0
                    if is_same_domain(root_url, resolved_normalized_url) and not should_skip_url(resolved_normalized_url):
